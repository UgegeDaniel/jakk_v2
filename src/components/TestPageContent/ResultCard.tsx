  ...
  return (
    <Card style={{ width: '100%' }}>
      <div className="d-flex align-items-center justify-content-between flex-fill my-1 px-5">
        ...
        <span className="display-5 rounded-circle text-success font-weight-bold">{result.score}%</span>
      </div>
      <Card.Body>
        <Table striped bordered hover>
          ...
          <tbody>
            <tr>{tableContent.map((content, index) => <td key={index}> {content}</td>)}</tr>
          </tbody>
        </Table>
        <div className="d-flex align-items-center justify-content-between flex-fill my-1 mx-1">
          ...
          <Link to="/testparams">
            <Button variant="outline-primary">Take Another Test</Button>
          </Link>
        </div>
      </Card.Body>
    </Card>
